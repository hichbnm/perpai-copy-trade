...
async def check_api_key_required(interaction: discord.Interaction, bot) -> bool:
    ...
    if bot.db.is_user_banned(user_id):
        banned_message = (
            ...
            f"📧 **For appeals, contact server staff.**"
        )
    ...
    if not api_keys:
        ...
        setup_message = (
            ...
            f"**🛠️ Setup Steps:**\n"
            f"1️⃣ Use `/add_api_key` command\n"
            f"2️⃣ Enter your exchange credentials\n"
...
class BackToDashboardView(discord.ui.View):
    ...
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        message = (
            ...
            f"⚙️  **Setup & Configuration** - Manage API keys and subscriptions\n"
            f"💰 **Trading Dashboard** - Balance, positions & signal testing\n"
            f"📊 **Analytics & Reports** - Performance tracking and insights\n"
            ...
            f"3️⃣ Automated trading begins!\n"
            f"4️⃣ Monitor trades in real-time\n\n"
            f"**� System Status:**\n"
            f"🟢 Bot Online\n"
...
class BackToSetupView(discord.ui.View):
    ...
    @discord.ui.button(label="← Back to Setup", style=discord.ButtonStyle.secondary, emoji="⚙️")
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Simple message
        message = (
            ...
            f"**📋 Available Options:**\n"
            f"🔑 **API Key Management** - Add and manage exchange credentials\n"
            f"📢 **Channel Subscriptions** - Subscribe to signal channels\n"
            f"📋 **View Settings** - Review your current configuration\n\n"
            f"**� Security Features:**\n"
            f"▸ All credentials are encrypted\n"
            f"▸ Secure database storage\n"
            f"▸ Zero data sharing policy"
        )
...
class DashboardView(discord.ui.View):
    ...
    async def setup_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        message = f"""⚙️ **SETUP & CONFIGURATION**
...
───────────────────────────
Auto Trade Bot • Secure Configuration Panel"""
        
        view = SetupView(self.bot)
        await interaction.response.edit_message(content=message, embed=None, view=view, attachments=[])
    
    @discord.ui.button(label="Trading", style=discord.ButtonStyle.success, emoji="💰", row=0)
    async def trading_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text="Auto Trade Bot • Trading Control Center")
    ...
    async def analytics_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        message = f"""📊 **ANALYTICS & PERFORMANCE**
...
───────────────────────────
Auto Trade Bot • Performance Analytics"""
        
        view = AnalyticsView(self.bot)
        await interaction.response.edit_message(content=message, embed=None, view=view, attachments=[])
    
    @discord.ui.button(label="Monitor", style=discord.ButtonStyle.secondary, emoji="🔍", row=1)
    ...
    async def help_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        commands = (
            ...
            "▸ `/subscribe` - Quick subscribe to current channel\n"
            "▸ `/balance` - Check account balance\n"
            "▸ `/positions` - View open positions\n"
            "▸ `/history` - Trade execution history"
        )
        ...
        embed.set_footer(text="Auto Trade Bot • Need more help? Contact support")
...
class ExchangeSelectionView(discord.ui.View):
    ...
    @discord.ui.button(label="← Back to Setup", style=discord.ButtonStyle.secondary, emoji="⚙️", row=1)
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class ExchangeDropdown(discord.ui.Select):
    def __init__(self, bot, parent_view):
        ...
        options = [
            ...
            discord.SelectOption(
                label="Bybit",
                description="Centralized crypto derivatives exchange",
                emoji="🟡",
                value="bybit"
            ),
            discord.SelectOption(
...
class RiskManagementView(discord.ui.View):
    ...
    @discord.ui.button(label="Update Risk Settings", style=discord.ButtonStyle.primary, emoji="⚙️", row=0)
    ...
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        message = (
            ...
            f"📢 **Channel Subscriptions** - Subscribe to signal channels\n"
            f"📋 **View Settings** - Review your current configuration\n"
            f"🛡️ **Risk Management** - Update position sizing & risk\n\n"
            f"**🔐 Security Features:**\n"
            f"▸ All credentials are encrypted\n"
            f"▸ Secure database storage\n"
            f"▸ Zero data sharing policy"
...
class RiskManagementModal(discord.ui.Modal):
    ...
    def __init__(self, bot):
        super().__init__(title="🛡️ Update Risk Settings")
        self.bot = bot
    
    amount = discord.ui.TextInput(
        label="💵 Trade Amount (in $)",
        placeholder="Example: 100 means trade with $100 per signal",
        default="100",
        max_length=10,
        required=True
    )
    ...
    max_risk = discord.ui.TextInput(
        label="⚠️ Max Loss Limit (% of Balance)",
        placeholder="Example: 2 means never lose more than 2% of balance",
        default="2",
        max_length=10,
        required=True
    )
        ...
        try:
            ...
            try:
                ...
                await interaction.response.send_message(
                    "❌ Amount and Max Risk must be valid numbers",
                    ephemeral=True
                )
                return
            ...
            if max_risk_val <= 0:
                await interaction.response.send_message(
                    "❌ Max loss limit must be greater than 0%",
                    ephemeral=True
                )
                return
            ...
            if not subscriptions:
                await interaction.response.send_message(
                    "❌ No subscriptions found to update",
                    ephemeral=True
                )
                return
            
            # Update all subscriptions with fixed mode
            updated_count = 0
            for sub in subscriptions:
                try:
                    success = self.bot.db.update_subscription(
                        subscription_id=sub['id'],
                        position_mode='fixed',  # Always use fixed mode now
            ...
            success_message = (
                ...
                f"**New Settings:**\n"
                f"💵 Trade Amount: **${amount:.2f}** per signal\n"
                f"🛡️ Max Loss: **{max_risk_val}%** of balance\n\n"
                f"**📝 Effect:**\n"
                f"▸ All future signals will use these settings\n"
                f"▸ Leverage from signals is applied automatically\n"
                f"▸ Max loss safety cap prevents excessive losses\n"
                f"▸ Applied to all {updated_count} subscribed channels"
            )
            ...
            await interaction.response.send_message(
                f"❌ Error updating risk settings: {str(e)}",
                ephemeral=True
            )
    ...
    def __init__(self, bot, api_keys):
        ...
        for exchange in all_exchanges:
            ...
            if exchange in user_exchanges:
                ...
                description = f"Update {exchange} API key ({env}) ✅"
                emoji = "✅"
            else:
                env = "🔴 Live"  # Default to mainnet
...
class UpdateAPIKeyModal(discord.ui.Modal):
    ...
    def __init__(self, bot, exchange: str, testnet: bool):
        env = "Testnet" if testnet else "Live"
        super().__init__(title=f"🔄 Update {exchange.capitalize()} ({env})")
        self.bot = bot
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if self.exchange.lower() == 'hyperliquid':
                ...
                if not wallet.startswith('0x') or len(wallet) != 42:
                    await interaction.edit_original_response(
                        content="❌ Invalid wallet address format. Must start with 0x and be 42 characters."
                    )
                    return
                ...
                if len(private_key) != 64:
                    await interaction.edit_original_response(
                        content="❌ Invalid private key format. Must be 64 hex characters."
                    )
                    return
                
                # ✅ VALIDATE CREDENTIALS WITH HYPERLIQUID
                try:
                    ...
                    if not validation_result['valid']:
                        await interaction.edit_original_response(
                            content=f"❌ **Credential Validation Failed**\n\n"
                                    f"{validation_result['message']}\n\n"
                                    f"**Please check:**\n"
                                    f"▸ Wallet address is correct\n"
                                    f"▸ Private key matches the wallet\n"
                    ...
                    logger.info(f"✅ Credentials validated for {wallet[:10]}... (Balance: ${balance:.2f})")
                ...
                if not success:
                    await interaction.edit_original_response(
                        content="❌ This wallet address is already in use by another user."
                    )
                    return
                
            else:
                # Bybit and other exchanges
                ...
                if self.exchange.lower() == 'bybit':
                    try:
                        ...
                        is_connected = await connector.connect(test_creds)
                        
                        if not is_connected:
                            await interaction.edit_original_response(
                                content=f"❌ **Bybit Credential Validation Failed**\n\n"
                                        f"Could not connect to Bybit with the provided credentials.\n\n"
                                        f"**Please check:**\n"
                                        f"▸ API Key is correct\n"
                                        f"▸ API Secret is correct\n"
                                        f"▸ API key has required permissions (Trade, Read)\n"
                                        f"▸ You're using the correct network ({'Testnet' if self.testnet else 'Mainnet'})"
                            )
                            return
                        
                        # Get balance to confirm working credentials
                        balance = await connector.get_balance(test_creds)
                        total_balance = balance.get('USDT', 0) if isinstance(balance.get('USDT'), (int, float)) else balance.get('coins', {}).get('USDT', {}).get('equity', 0)
                        
                        logger.info(f"✅ Bybit credentials validated (Balance: ${total_balance:.2f})")
                        
                    except Exception as val_err:
                        logger.error(f"Bybit validation error: {val_err}")
                        await interaction.edit_original_response(
                            content=f"❌ **Bybit Validation Error**\n\n"
                                    f"Unable to validate credentials: {str(val_err)}\n\n"
                                    f"Please check your API key and secret are correct."
                        )
                        return
                ...
                if not success:
                    await interaction.edit_original_response(
                        content="❌ This API key is already in use by another user."
                    )
                    return
            ...
            success_message = (
                ...
                f"🏦 **Exchange:** {self.exchange.capitalize()}\n"
                f"**Environment:** {env}\n"
            )
            
            if self.exchange.lower() == 'hyperliquid' and 'balance' in locals():
                success_message += f"💰 **Balance:** ${balance:.2f}\n"
            
            success_message += (
                f"\n**✅ Status:** Your API key has been validated and is now active.\n\n"
                f"**🚀 Next Steps:**\n"
                f"▸ Your existing subscriptions will use the new key\n"
                f"▸ New trades will execute with updated credentials\n"
                f"▸ Return to dashboard to continue"
            )
...
class SetupView(discord.ui.View):
    def __init__(self, bot):
        super().__init__(timeout=300)
        self.bot = bot
        # Enforce button order and new row layout after swaps requested by user:
        # - Swap 'View Settings' with 'Subscribe'
        # - Swap 'Unsubscribe All' with 'Change API Key'
        # Final desired rows/orders:
        #   Row 1: [View Settings, View Subscriptions]
        #   Row 2: [Subscribe, Unsubscribe All]
        #   Row 3: [Risk Management, Change API Key]
        try:
            # Helper to (re)order a row by labels if present
            def reorder_row(desired_labels, row_index):
                present = {btn.label: btn for btn in list(self.children) if isinstance(btn, discord.ui.Button) and btn.row == row_index}
                # Remove all present in desired, then add back in desired order
                for label in desired_labels:
                    btn = present.get(label)
                    if btn:
                        self.remove_item(btn)
                for label in desired_labels:
                    btn = present.get(label)
                    if btn:
                        # Ensure row is correct (discord keeps the row on the Button instance)
                        btn.row = row_index
                        self.add_item(btn)

            # Apply ordering per row
            reorder_row(["Add API Key", "Change API Key"], 0)
            reorder_row(["View Settings", "View Subscriptions"], 1)
            reorder_row(["Subscribe", "Unsubscribe All"], 2)
            reorder_row(["Risk Management"], 3)
        except Exception as _order_err:
            # Non-fatal; UI will still work even if reordering fails
            pass
    
    # Row 0: Primary Actions - API Key Management
    @discord.ui.button(label="Add API Key", style=discord.ButtonStyle.success, emoji="🔑", row=0)
    async def add_api_key(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if existing_keys:
            ...
            message = (
                ...
                f"⚠️ You already have API keys configured:\n\n"
            )
            
            for key_data in existing_keys:
                exchange = key_data['exchange']
                testnet = key_data['testnet']
                env = "🧪 Testnet" if testnet else "🔴 Live Trading"
                message += f"🏦 **{exchange.capitalize()}** - {env}\n"
            
            message += (
                f"\n**⚠️ Note:**\n"
                f"Currently, only one API key per exchange is supported.\n\n"
                f"**Options:**\n"
                f"▸ Use 'View Settings' to see your current configuration\n"
                f"▸ Contact admin to update or remove existing keys\n"
                f"▸ Return to setup menu"
            )
    ...
    # (Removed duplicate/incorrectly labeled buttons to avoid conflicts)
    
    # Row 2: Configuration & Settings
    @discord.ui.button(label="Change API Key", style=discord.ButtonStyle.secondary, emoji="🔄", row=0)
        ...
        try:
            ...
            if not api_keys_data:
                message = (
                    ...
                    "• All subscriptions will use the latest key\n\n"
                    "**📝 Select exchange below:**"
                )
            ...
            for key_data in api_keys_data:
                ...
                env = "🧪 Testnet" if testnet else "🔴 Live"
                message += f"✅ **{exchange.capitalize()} - {env}** (Active)\n"
                ...
                if exchange.lower() == 'hyperliquid':
                    ...
                    if wallet and len(wallet) == 42 and wallet.startswith('0x'):
                        ...
                        message += f"   📍 Wallet: `{wallet[:6]}...{wallet[-4:]}`\n"
                    else:
                        message += f"   📍 Wallet: `{wallet or 'Not available'}`\n"
                    message += f"   🔐 Private Key: `Protected (not visible)`\n"
                elif api_key:
                    # For other exchanges, show partial API key
                    if len(api_key) > 10:
                        message += f"   🔑 API Key: `{api_key[:8]}...{api_key[-4:]}`\n"
                    else:
                        message += f"   🔑 API Key: `{api_key[:4]}...`\n"
                    message += f"   🔐 Secret: `Protected (not visible)`\n"
            ...
            message += (
                ...
                "• All subscriptions will use the new exchange\n"
                "• Private keys are encrypted and never displayed\n\n"
                "**📝 Select exchange (update current or switch to another):**"
            )
            ...
            error_message = (
                "🔄 **Change API Key**\n\n"
                "⚠️ Error loading API keys.\n"
                "Please try again or contact support."
            )
            view = BackToSetupView(self.bot)
            await interaction.response.edit_message(content=error_message, embed=None, view=view)

    @discord.ui.button(label="View Settings", style=discord.ButtonStyle.secondary, emoji="📋", row=1)
    async def view_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            settings_message = f"📋 **Configuration Settings**\n{interaction.user.mention}\n\n"
            
            # API Keys Section
            settings_message += "🔑 **API Keys**\n"
            ...
            if api_keys_data:
                for key_data in api_keys_data:
                    ...
                    testnet = key_data['testnet']
                    env = "🧪 Testnet" if testnet else "🔴 Live"
                    settings_message += f"• {exchange.capitalize()} ({env}) - ✅ Active\n"
            else:
                settings_message += "❌ None configured\n"
            ...
            if subscriptions:
                ...
                settings_message += "❌ No active subscriptions\n"

            # Trading Status
            settings_message += "\n📊 **Status**\n"
            
            if api_keys_data and subscriptions:
                settings_message += "✅ Fully configured & ready to trade\n"
            elif api_keys_data:
                settings_message += "⚠️ API keys added - subscribe to channels\n"
            elif subscriptions:
                settings_message += "⚠️ Channels subscribed - add API keys\n"
            else:
                settings_message += "❌ Setup required - add keys & subscribe\n"
    ...
    @discord.ui.button(label="Subscribe", style=discord.ButtonStyle.primary, emoji="📢", row=2)
    async def subscribe(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(
            name="",
            value="📢 **Channel Subscription Manager**\nSelect a category to view channels",
            inline=False
        )
        ...
        server_info = (
            f"▸ Server: **{guild.name}**\n"
            f"▸ Categories found: {categories_count}\n"
        ...
        embed.set_footer(text="Auto Trade Bot • Category Selection")
        
        await interaction.response.edit_message(embed=embed, view=view)
    
    @discord.ui.button(label="View Subscriptions", style=discord.ButtonStyle.secondary, emoji="📋", row=1)
        ...
        try:
            ...
            if not subscriptions:
                message = (
                    ...
                    f"**🚀 Get Started:**\n"
                    f"▸ Click **Subscribe** button above\n"
                    f"▸ Select channels to monitor\n"
                    f"▸ Start automated trading"
                )
            else:
                message = (
                ...
                message += "✅ **Status:** All channels actively monitoring for trading signals"
    ...
    # Row 2: Configuration & Settings
    @discord.ui.button(label="Risk Management", style=discord.ButtonStyle.primary, emoji="🛡️", row=3)
        ...
        try:
            ...
            if not subscriptions:
                message = (
                    ...
                    "**📝 Next Steps:**\n"
                    "▸ Click 'Subscribe' button to add channels\n"
                    "▸ Then you can manage risk settings"
                )
                view = BackToSetupView(self.bot)
                await interaction.response.edit_message(content=message, embed=None, view=view)
                return
            ...
            # Show average settings across all subscriptions
            avg_amount = sum(s.get('fixed_amount', 100.0) for s in subscriptions) / len(subscriptions)
            avg_risk = sum(s.get('max_risk', 2.0) for s in subscriptions) / len(subscriptions)
            
            message += f"• Average Trade Amount: ${avg_amount:.2f}\n"
            message += f"• Average Max Loss: {avg_risk:.1f}%\n"
            
            message += (
                "\n**⚙️ What You Can Change:**\n"
                "• Trade amount per signal (in $)\n"
                "• Max loss limit (% of balance)\n\n"
                "**⚠️ Important:**\n"
                "• Changes apply to ALL subscribed channels\n"
                "• Existing trades are not affected\n"
                "• New signals will use updated settings\n"
                "• Leverage from signals is applied automatically\n\n"
                "**📝 Click below to update risk settings:**"
            )
    ...
    # Row 3: Danger Zone
    @discord.ui.button(label="Unsubscribe All", style=discord.ButtonStyle.danger, emoji="❌", row=2)
    async def unsubscribe_all(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            success_text = (
                ...
                f"🗑️ **Removed**: {removed_count} subscriptions\n\n"
                f"You will no longer receive automated trades from any channels.\n\n"
                f"**🔄 What's Next?**\n"
                f"▸ Use **Subscribe** button to add new channels\n"
                f"▸ Configure risk settings\n"
    ...
    # Row 4: Navigation
    @discord.ui.button(label="← Back to Dashboard", style=discord.ButtonStyle.secondary, emoji="🏠", row=4)
...
class TradingView(discord.ui.View):
    ...
    @discord.ui.button(label="Check Balance", style=discord.ButtonStyle.success, emoji="💵", row=0)
    async def check_balance(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not api_keys:
                ...
                message = (
                    "⚠️ **No API Key Found**\n\n"
                    "📝 **Required:** Add your exchange API key first using the **Setup** menu.\n\n"
                    "🔧 **Quick Fix:**\n"
                    "▸ Return to dashboard\n"
                    "▸ Click **Setup** button\n"
            ...
            # Show loading state
            loading_message = "💰 **Checking Balance...**\n⏳ Please wait..."
            ...
            account_type = '🧪 Testnet Mode' if first_key.get('testnet') else '🔴 Live Trading'
            
            message = (
                f"💰 **Account Balance** - {exchange.capitalize()} Exchange\n\n"
                f"╭─ **Total Balance**\n"
                f"│  ${balance.get('total', 0):,.2f} USD\n"
                f"│\n"
                f"├─ **Available**\n"
                f"│  ${balance.get('available', 0):,.2f} USD\n"
                f"│\n"
                f"╰─ **Account Type**\n"
                f"   {account_type}\n\n"
                f"✅ Balance updated\n"
    ...
    @discord.ui.button(label="Back to Dashboard", style=discord.ButtonStyle.secondary, emoji="🔙", row=1)
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class AnalyticsDetailView(discord.ui.View):
    ...
    @discord.ui.button(label="7 Days", style=discord.ButtonStyle.secondary, emoji="📅", row=0)
    ...
    @discord.ui.button(label="30 Days", style=discord.ButtonStyle.primary, emoji="📅", row=0)
    ...
    @discord.ui.button(label="90 Days", style=discord.ButtonStyle.secondary, emoji="📅", row=0)
    ...
    @discord.ui.button(label="All Time", style=discord.ButtonStyle.secondary, emoji="♾️", row=0)
    ...
    async def _show_analytics(self, interaction: discord.Interaction, days: int):
        ...
        try:
            ...
            if metrics.get('total_trades', 0) == 0:
                message = (
                    f"📊 **Performance Analytics** ({days} Days)\n"
                    f"❌ No completed trades in this period"
                )
            else:
                # Generate report
            ...
            await interaction.followup.send(
                f"❌ Error loading {days}-day analytics",
                ephemeral=True
            )
...
class AnalyticsView(discord.ui.View):
    ...
    @discord.ui.button(label="Trade History", style=discord.ButtonStyle.primary, emoji="📈", row=0)
    async def trade_history(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Show loading state
        loading_message = "📈 **Loading Trade History...**\n⏳ Please wait..."
        await interaction.response.edit_message(content=loading_message, embed=None, view=None)
        ...
        try:
            ...
            if not trades:
                message = (
                    ...
                    "**🚀 How to Trade:**\n"
                    "1️⃣ Add API keys in Setup\n"
                    "2️⃣ Subscribe to signal channels\n"
                    "3️⃣ Post or wait for signals\n"
                    "4️⃣ Trades execute automatically"
                )
            else:
                ...
                for i, trade in enumerate(trades[:5], 1):
                    ...
                    date_raw = trade.get('created_at', 'Unknown')
                    
                    # Format date - handle datetime objects
    ...
    @discord.ui.button(label="Performance", style=discord.ButtonStyle.secondary, emoji="📊", row=0)
    async def performance_stats(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Show loading state
        loading_message = "📊 **Loading Performance Analytics...**\n⏳ Analyzing your trading data..."
        await interaction.response.edit_message(content=loading_message, embed=None, view=None)
        ...
        try:
            ...
            if metrics.get('total_trades', 0) == 0:
                message = (
                    "📊 **Performance Analytics**\n"
                    "❌ No completed trades in last 30 days\n\n"
                    "**ℹ️ Info:**\n"
                ...
                message = f"""📊 **PERFORMANCE ANALYTICS** (30 Days)
{rating_color} **{rating}**
...
**📉 Risk Metrics:**
╭─ Max Drawdown: -${metrics['max_drawdown']['max_drawdown']:.2f}
│  ({metrics['max_drawdown']['max_drawdown_percent']:.1f}%)
...
class CategorySelectionView(discord.ui.View):
    ...
    def __init__(self, bot, guild, user, dashboard_message=None):
        ...
        # Add back button
        back_btn = discord.ui.Button(label="← Back to Setup", style=discord.ButtonStyle.secondary, emoji="⚙️")
    ...
    async def back_to_setup(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="📋 Available Options", value=setup_options, inline=False)
        ...
        embed.add_field(name="🔐 Security Features", value=security, inline=False)
        embed.set_footer(text="Auto Trade Bot • Secure Configuration Panel")
...
class CategoryDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        tips = (
            "💡 **How to Select**:\n"
            "▸ **Select All**: Subscribe to all channels at once\n"
            "▸ **Dropdown**: Choose specific channels (up to 10)\n"
            "▸ **Clear All**: Reset your selection\n"
            "▸ Same settings apply to all selected channels"
        )
        ...
        embed.set_footer(text="Auto Trade Bot • Multi-Channel Selection")
...
class ChannelsInCategoryView(discord.ui.View):
    ...
    def __init__(self, bot, channels, category_name, guild, user, dashboard_message_id=None, dashboard_channel_id=None):
        ...
        select_all_btn = discord.ui.Button(label="✅ Select All Channels", style=discord.ButtonStyle.success, emoji="✅", row=1)
        select_all_btn.callback = self.select_all_channels
        self.add_item(select_all_btn)
        
        # Add back to categories button
        back_btn = discord.ui.Button(label="← Back to Categories", style=discord.ButtonStyle.secondary, emoji="📁", row=1)
    ...
    async def select_all_channels(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not all_channel_ids:
                await interaction.response.send_message("❌ No channels to select!", ephemeral=True)
                return
            
            # Show confirmation
            channel_list = "\n".join([f"• #{channel.name}" for channel in self.channels[:10]])
            ...
            await interaction.response.send_message("❌ Error selecting all channels. Please try again.", ephemeral=True)
...
class ChannelInCategoryDropdown(discord.ui.Select):
    ...
    def __init__(self, bot, channels, category_name, parent_view=None):
        ...
        super().__init__(
            placeholder=f"📢 Select channel(s) from {category_name}...",
            min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if len(self.values) == 1:
            ...
            if not channel:
                ...
                await interaction.response.send_message(f"❌ Channel not found! ID: {selected_channel_id}", ephemeral=True)
                return
...
class ChannelSelectionView(discord.ui.View):
    def __init__(self, bot, guild=None, requester: discord.abc.User | None = None):
        ...
        self.previous_button = discord.ui.Button(label="◀️ Previous", style=discord.ButtonStyle.secondary, disabled=True)
        self.next_button = discord.ui.Button(label="Next ▶️", style=discord.ButtonStyle.secondary)
        self.refresh_button = discord.ui.Button(label="🔄 Refresh", style=discord.ButtonStyle.success, emoji="🔄")
    ...
    async def refresh_channels_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if success:
                ...
                embed.add_field(
                    name="📋 Next Steps",
                    value="• Select channels from the dropdown\n• Use navigation buttons for more channels\n• Configure your trading settings\n• Start receiving trading signals",
                    inline=False
                )
            else:
                embed.add_field(
                    name="⚠️ No Channels Found",
            ...
            logger.error(f"Error refreshing channels: {e}")
            await interaction.followup.send("❌ Error refreshing channels. Please try again.", ephemeral=True)
...
class ChannelSelectDropdown(discord.ui.Select):
    def __init__(self, bot, guild=None, requester: discord.abc.User | None = None, page=0, per_page=10):
        ...
        super().__init__(
            placeholder="🔽 Select channels to subscribe to...",
            min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        if "none" in self.values:
            await interaction.response.send_message("❌ No channels available to subscribe to.", ephemeral=True)
            return
...
class APIKeyModal(discord.ui.Modal):
    def __init__(self, bot, view, exchange="hyperliquid"):
        super().__init__(title=f"🔑 Add API Key - {exchange.capitalize()}")
        self.bot = bot
        self.view = view
        ...
        try:
            ...
            if exchange_name == 'hyperliquid':
                ...
                success_text = (
                    ...
                    f"🏦 Exchange: **{exchange_name.capitalize()}**\n"
                    f"🔴 Environment: Live Trading\n"
                    f"💰 Balance: ${balance:.2f}\n"
                    f"🔒 Security: Encrypted & Secure\n\n"
                    f"🔄 You can now check balance, subscribe to channels, and start trading!"
                )
                ...
                if not success:
                    await interaction.edit_original_response(
                        content=f"❌ This API key is already registered by another user."
                    )
                    return
            ...
            try:
                await interaction.edit_original_response(
                    content=f"❌ Error adding API key: {str(e)}\nPlease try again."
                )
            except:
                await interaction.response.send_message(
                    f"❌ Error adding API key: {str(e)}\nPlease try again.",
                    ephemeral=True
                )

        except Exception as e:
            logger.error(f"API key error: {e}")
            await interaction.response.send_message("❌ Error adding API key. Please try again.", ephemeral=True)

class SubscribeModal(discord.ui.Modal):
    def __init__(self, bot, view=None):
        super().__init__(title="📢 Subscribe to Channel - Simplified")
        self.bot = bot
        self.view = view
    ...
    position_amount = discord.ui.TextInput(
        label="💵 Trade Amount (in $)",
        placeholder="Example: 100 means trade with $100 per signal",
        default="100",
        max_length=10,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Get user's most recent API key to determine exchange
            user_id = str(interaction.user.id)
            api_keys = self.bot.db.get_user_all_api_keys(user_id)
            ...
            if not api_keys:
                await interaction.response.send_message(
                    "❌ **No API Key Found**\n\n"
                    "You need to add an API key first using `/add_api_key`\n"
                    "The subscription will use the exchange from your most recent API key.",
                    ephemeral=True
                )
                return
            
            # Get the most recent API key (last one in the list)
            latest_api_key = api_keys[-1]
            exchange_value = latest_api_key.get('exchange', 'hyperliquid').lower()
            
            logger.info(f"Auto-detected exchange: {exchange_value} from user's latest API key")
            
            # Parse and validate inputs
            try:
                amount_val = float(self.position_amount.value)
            ...
            # Validate max risk (must be positive)
            if max_risk_val <= 0:
                await interaction.response.send_message(
                    "❌ **Invalid Max Risk**\n\nMax risk must be greater than 0%",
                    ephemeral=True
                )
                return
            ...
            if existing_subscription:
                ...
                existing_amount = existing_subscription.get('fixed_amount', 100.0)
                existing_risk = existing_subscription.get('max_risk', 2.0)
                
                await interaction.response.send_message(
                    f"⚠️ **Already Subscribed!**\n\n"
                    f"You are already subscribed to **#{target_channel_name}**\n\n"
                    f"📊 Current Settings:\n"
                    f"🏦 Exchange: **{existing_subscription.get('exchange', 'N/A').capitalize()}**\n"
                    f"💵 Trade Amount: **${existing_amount:.2f}** per trade\n"
                    f"⚠️ Max Risk: **{existing_risk}%** maximum loss\n\n"
                    f"💡 To change settings, unsubscribe first and then resubscribe.",
                    ephemeral=True
                )
                return
            
            api_key = self.bot.db.get_user_api_key(str(interaction.user.id), exchange_value)
            if not api_key:
                # This shouldn't happen since we already validated API key exists, but just in case
                await interaction.response.send_message(
                    f"❌ **API Key Error**\n\n"
                    f"Could not retrieve API key for exchange: **{exchange_value.capitalize()}**\n\n"
                    f"Please try adding your API key again.",
                    ephemeral=True
                )
                return
            ...
            # Subscribe with simplified fixed-amount system
            mode = 'fixed'  # Always use fixed mode now
            fixed_amount = amount_val
            percentage = 10.0  # Legacy field, not used
            ...
            success_text = (
                ...
                f"🏦 Exchange: **{exchange_value.capitalize()}**\n"
                f"💵 Trade Amount: **${amount_val:.2f}** per signal\n"
                f"🛡️ Max Loss: **{max_risk_val}%** of account balance\n\n"
                f"📝 **How it works:**\n"
                f"• Each signal trades with ${amount_val:.2f} (before leverage)\n"
                f"• Leverage from signal is applied automatically\n"
                f"• Max loss safety cap prevents losses exceeding {max_risk_val}%\n"
                f"• Stop loss reduces position if needed\n\n"
                f"🚀 Trading signals posted in this channel will now trigger automatic trades!"
            )
            ...
            await interaction.response.send_message("❌ Please enter valid numbers for position size and max risk.", ephemeral=True)
        except Exception as e:
            logger.error(f"Subscribe error: {e}")
            await interaction.response.send_message("❌ Error subscribing to channel. Please try again.", ephemeral=True)

class MultiChannelSubscribeModal(discord.ui.Modal):
    def __init__(self, bot, channel_ids, guild):
        super().__init__(title="📢 Subscribe - Simplified")
        self.bot = bot
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            logger.info(f"Multi-channel auto-detected exchange: {exchange_value} from user's latest API key")
            ...
            # Validate amount
            if amount_val <= 0:
                await interaction.response.send_message("❌ Trade amount must be > $0", ephemeral=True)
                return
            
            # Validate max risk (must be positive)
            if max_risk_val <= 0:
                await interaction.response.send_message("❌ Max risk must be > 0%", ephemeral=True)
                return
            
            # Check API key (should exist since we already validated above)
            api_key = self.bot.db.get_user_api_key(str(interaction.user.id), exchange_value)
            if not api_key:
                # This shouldn't happen since we already validated API key exists, but just in case
                await interaction.response.send_message(
            ...
            # Use simplified fixed mode
            mode = 'fixed'
            position_size_val = 1.0  # Legacy
            fixed_amount = amount_val
            percentage = 10.0  # Legacy
                ...
                try:
                    ...
                    if channel and isinstance(channel, discord.TextChannel):
                        ...
                        if bot_member and channel.permissions_for(bot_member).read_messages:
                            # Check if user is already subscribed to this channel
                            existing_subscription = self.bot.db.get_subscription(str(interaction.user.id), str(channel.id))
                            ...
                            if existing_subscription:
                                ...
                                logger.info(f"Updated subscription for user {interaction.user.id} to channel #{channel.name}")
                            else:
            ...
            success_text = f"✅ **Multi-Channel Subscription Results**\n\n"
            
            if subscribed_channels:
                success_text += f"✅ **New Subscriptions** ({len(subscribed_channels)}):\n"
                for channel_name in subscribed_channels:
                    success_text += f"• #{channel_name} ✅\n"
                success_text += f"\n"
            
            if updated_channels:
                success_text += f"🔄 **Updated Subscriptions** ({len(updated_channels)}):\n"
                for channel_name in updated_channels:
                    success_text += f"• #{channel_name} 🔄\n"
                success_text += f"\n"
            
            if failed_channels:
                success_text += f"❌ **Failed Subscriptions** ({len(failed_channels)}):\n"
                for channel_id in failed_channels:
                    try:
                        ...
                        success_text += f"• Unknown Channel ❌\n"
                success_text += f"\n"
            
            success_text += f"⚙️ **Settings Applied**:\n"
            success_text += f"🏦 Exchange: **{exchange_value.capitalize()}**\n"
            success_text += f"💵 Trade Amount: **${amount_val:.2f}** per signal\n"
            success_text += f"🛡️ Max Loss: **{max_risk_val}%** of balance\n\n"
            
            if subscribed_channels or updated_channels:
                total_active = len(subscribed_channels) + len(updated_channels)
                success_text += f"🚀 **Ready!** Trading signals in {total_active} channels will trigger automatic trades!"
            
            if failed_channels and not subscribed_channels and not updated_channels:
                success_text += f"⚠️ **No channels were successfully subscribed.** Please try again or check permissions."
            ...
            await interaction.response.send_message("❌ Error subscribing to channels. Please try again.", ephemeral=True)
...
class CleanUICommands(commands.Cog):
    ...
    async def dashboard(self, interaction: discord.Interaction):
        try:
            ...
            quick_start = (
                "1️⃣ Click **Setup** to add your API key\n"
                "2️⃣ Subscribe to signal channels\n"
                "3️⃣ Automated trading begins!\n"
                "4️⃣ Monitor trades in real-time"
            )
            ...
            embed.set_footer(
                text=f"Auto Trade Bot v2.0 • User: {interaction.user.name} • #{interaction.channel.name}",
            ...
            message_text = "❌ Dashboard UI error. Use `/ping` to test or other commands directly."
    ...
    async def ping(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if interaction.response.is_done():
                await interaction.followup.send("🏓 Pong! Bot is working!", ephemeral=False)
            else:
    ...
    async def status(self, interaction: discord.Interaction):
        try:
            ...
            try:
                ...
                if interaction.response.is_done():
                    await interaction.followup.send("✅ Bot is running!", ephemeral=interaction.guild is not None)
                else:
                    await interaction.response.send_message("✅ Bot is running!", ephemeral=interaction.guild is not None)
    ...
    async def quick_balance(self, interaction: discord.Interaction, exchange: str = "hyperliquid"):
        try:
            ...
            embed.add_field(name="💵 Total", value=f"${balance.get('total', 0):.2f}", inline=True)
            embed.add_field(name="💸 Available", value=f"${balance.get('available', 0):.2f}", inline=True)
            embed.add_field(name="🏦 Account", value=f"{'🧪 Testnet' if api_key_data.get('testnet') else '🔴 Live'}", inline=True)
...
class MonitoringView(discord.ui.View):
    ...
    @discord.ui.button(label="📊 My Active Trades", style=discord.ButtonStyle.success)
    async def my_trades(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not active_trades:
                message_text = f"""📊 **YOUR ACTIVE TRADES**
...
"""
                ...
                if len(active_trades) > 10:
                    message_text += f"📋 **More Trades**\n... and {len(active_trades) - 10} more trades\n\n"
                
                message_text += "🔄 Real-time monitoring with automatic alerts"
...
Please try again or contact support."""
    ...
    @discord.ui.button(label="⏹️ Service Control", style=discord.ButtonStyle.secondary)
    ...
    async def back_to_dashboard(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            title="🤖 Discord Trading Bot Dashboard",
        ...
        embed.add_field(
            name="🚀 Quick Start",
            value="• Add API keys in Setup\n• Subscribe to signal channels\n• Monitor live trades\n• Track performance",
            inline=False
        )
        embed.add_field(
            name="📊 Real-time Features",
            value="• Instant signal processing\n• Live price monitoring\n• Automatic TP/SL alerts\n• Multi-channel support",
            inline=False
        )
        embed.set_footer(text="🔥 Built for professional traders | Real-time monitoring active")
...
class ServiceControlView(discord.ui.View):
    ...
    @discord.ui.button(label="🔄 Restart Service", style=discord.ButtonStyle.primary)
    ...
    @discord.ui.button(label="🔙 Back to Monitoring", style=discord.ButtonStyle.secondary)
...
class PermanentDashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="⚙️ Setup", style=discord.ButtonStyle.primary, emoji="⚙️", custom_id="persistent_setup")
    async def setup_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Check if user is banned
        user_id = str(interaction.user.id)
        self.bot.db.add_user(user_id, interaction.user.name)
...
🚀 **Quick Actions:**
• Use `/add_api_key` to add your exchange credentials
...
🚀 **Quick Actions:**
• Use `/quick_balance` to check your balance
• Post signals in subscribed channels for auto-trading"""
        
        view = TradingView(self.bot)
        await interaction.response.send_message(trading_text, view=view, ephemeral=True)
    
    @discord.ui.button(label="📊 Analytics", style=discord.ButtonStyle.secondary, emoji="📊", custom_id="persistent_analytics")
    async def analytics_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Check if user is banned
        user_id = str(interaction.user.id)
        self.bot.db.add_user(user_id, interaction.user.name)
...
🚀 **Quick Actions:**
• Use `/dashboard` then Analytics for detailed stats
• Check trade history and performance metrics
• Monitor your trading success rate"""
        
        view = AnalyticsView(self.bot)
        await interaction.response.send_message(analytics_text, view=view, ephemeral=True)
    
    @discord.ui.button(label="🔍 Monitor", style=discord.ButtonStyle.secondary, emoji="🔍", custom_id="persistent_monitor")
    async def monitor_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Check if user is banned
        user_id = str(interaction.user.id)
        self.bot.db.add_user(user_id, interaction.user.name)
        ...
        try:
            ...
            monitoring_text = f"""🔍 **TRADE MONITORING STATUS**

Real-time trade monitoring and alerts

🔧 **Service Status**: {"🟢 Running" if status['service_running'] else "🔴 Stopped"}
📡 **Price Feed**: {"🟢 Connected" if status.get('price_feed_connected', False) else "🔴 Disconnected"}
📊 **Active Trades**: {status['active_trades']} monitored
📈 **Tracked Symbols**: {status.get('subscribed_symbols', 0)} symbols
🔄 **Update Frequency**: Every {status['update_interval']}s
📋 **Total Monitored**: {status['total_monitored']} trades
...
🚀 **Quick Actions:**
• Use `/dashboard` then Monitor for detailed view
• Your active trades are automatically monitored
• Alerts sent when targets are hit"""
            
        except Exception as e:
            monitoring_text = f"""🔍 **TRADE MONITORING**

Real-time trade monitoring system

⚠️ **Status**: Error retrieving monitoring status
📋 **Error**: {str(e)}

🚀 **Quick Actions:**
• Use `/dashboard` for detailed monitoring panel
• Contact admin if issues persist"""
        
        view = MonitoringView(self.bot)
        await interaction.response.send_message(monitoring_text, view=view, ephemeral=True)
    
    @discord.ui.button(label="❓ Help", style=discord.ButtonStyle.secondary, emoji="❓", custom_id="persistent_help")
    async def help_button(self, interaction: discord.Interaction, button: discord.ui.Button):
