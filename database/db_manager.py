...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # Clean up duplicate subscriptions before adding unique constraint
            cursor.execute('''
                DELETE FROM channel_subscriptions
                WHERE id NOT IN (
                    SELECT MIN(id)
                    FROM channel_subscriptions
                    GROUP BY user_id, channel_id, exchange
                )
            ''')
            rows_deleted = cursor.rowcount
            if rows_deleted > 0:
                logger.info(f"🧹 Cleaned up {rows_deleted} duplicate subscription(s)")
            
            # Add unique constraint to prevent duplicate subscriptions
            cursor.execute('''
                ALTER TABLE channel_subscriptions 
                DROP CONSTRAINT IF EXISTS channel_subscriptions_unique_user_channel_exchange
            ''')
            cursor.execute('''
                ALTER TABLE channel_subscriptions 
                ADD CONSTRAINT channel_subscriptions_unique_user_channel_exchange 
                UNIQUE (user_id, channel_id, exchange)
            ''')
    ...
    def subscribe_to_channel(self, user_id: str, channel_id: str, exchange: str,
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute('''
                ...
                ON CONFLICT ON CONSTRAINT channel_subscriptions_unique_user_channel_exchange
                DO UPDATE SET
    ...
    def log_trade(self, user_id: str, exchange: str, symbol: str, side: str,
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO trades (user_id, exchange, symbol, side, size, price, entry_price, signal_data, channel_id, message_id)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id
            ''', (user_id, exchange, symbol, side, size, price, price, signal_data, channel_id, message_id))
            inserted_id = cursor.fetchone()[0]
            logger.info(f"Trade logged: {user_id} {side} {size} {symbol} on {exchange} (ID: {inserted_id}, Entry: ${price})")
            return inserted_id
